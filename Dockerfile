...
ENV LANG="C.UTF-8" LANGUAGE="C.UTF-8" LC_ALL="C.UTF-8" TERM="xterm-256color" TZ="Asia/Seoul"

# Unminimize Ubuntu image and install core packages
RUN yes | unminimize \
    && DEBIAN_FRONTEND=noninteractive apt-get install -y \
        vim git ctags man bash-completion \
        openjdk-8-jdk-headless maven python3-pip build-essential \
        net-tools dnsutils iputils-ping iputils-tracepath tcpdump netcat \
        curl wget ssh ftp telnet rsync nmap whois \
        mysql-client zip unzip tree w3m \
# Install sbt
